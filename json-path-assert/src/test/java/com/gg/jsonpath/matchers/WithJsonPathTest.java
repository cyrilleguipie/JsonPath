...
import com.gg.jsonpath.InvalidPathException;
...
import static com.gg.jsonpath.matchers.JsonPathMatchers.withJsonPath;
import static com.gg.jsonpath.matchers.helpers.ResourceHelpers.resource;
...
public class WithJsonPathTest {
    ...
    public void shouldMatchExistingCompiledJsonPath() {
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.expensive")));
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.bicycle")));
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.book[2].title")));
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.book[*].author")));
    }
    ...
    public void shouldNotMatchNonExistingJsonPath() {
        assertThat(BOOKS_JSON, not(withJsonPath(JsonPath.compile("$.not_there"))));
        assertThat(BOOKS_JSON, not(withJsonPath(JsonPath.compile("$.store.book[5].title"))));
        assertThat(BOOKS_JSON, not(withJsonPath(JsonPath.compile("$.store.book[1].not_there"))));
    }
    ...
    public void shouldMatchJsonPathEvaluatedToStringValue() {
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.bicycle.color"), equalTo("red")));
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.book[2].title"), equalTo("Moby Dick")));
    ...
    public void shouldMatchJsonPathEvaluatedToIntegerValue() {
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.expensive"), equalTo(10)));
        assertThat(BOOKS_JSON, withJsonPath("$.expensive", equalTo(10)));
    }

    @Test
    public void shouldMatchJsonPathEvaluatedToDoubleValue() {
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.bicycle.price"), equalTo(19.95)));
    ...
    public void shouldMatchJsonPathEvaluatedToCollectionValue() {
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.book[*].author"), instanceOf(List.class)));
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.book[*].author"), hasSize(4)));
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$.store.book[*].author"), hasItem("Evelyn Waugh")));
        assertThat(BOOKS_JSON, withJsonPath(JsonPath.compile("$..book[2].title"), hasItem("Moby Dick")));
