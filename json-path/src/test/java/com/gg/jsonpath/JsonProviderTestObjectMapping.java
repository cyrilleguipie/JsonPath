...
public class JsonProviderTestObjectMapping extends BaseTest {
    ...
    public void list_of_numbers() {
        ...
        Assertions.assertThat(JsonPath.using(conf).parse(JSON_DOCUMENT).read("$.store.book[*].display-price", typeRef)).containsExactly(8.95D, 12.99D, 8.99D, 22.99D);
    }
    ...
    public void test_type_ref() throws IOException {
        ...
        Assertions.assertThat(JsonPath.using(conf).parse(JSON).read("$", typeRef)).extracting("foo").containsExactly("foo0", "foo1", "foo2");
    }
