...
public class MultiPropTest {
    ...
    public void multi_prop_can_be_read_from_root() {
        ...
        Assertions.assertThat(JsonPath.using(conf).parse(model).read("$['a', 'b']", Map.class))
                .containsEntry("a", "a-val")
                .containsEntry("b", "b-val");

        // current semantics: absent props are skipped
        Assertions.assertThat(JsonPath.using(conf).parse(model).read("$['a', 'd']", Map.class))
    ...
    public void non_leaf_multi_props_can_be_required() {
        ...
        assertThat((List) JsonPath.using(conf).parse(json).read("$['a', 'c'].v")).asList().containsOnly(5, 1);
