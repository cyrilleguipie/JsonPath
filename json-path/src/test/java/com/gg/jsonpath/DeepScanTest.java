...
import static com.gg.jsonpath.TestUtils.assertEvaluationThrows;
...
public class DeepScanTest extends BaseTest {
    ...
    public void when_deep_scanning_leaf_multi_props_work() {
        ...
        result = JsonPath.using(conf).parse("[{\"a\": \"a-val\", \"b\": \"b-val\", \"c\": \"c-val\"}, [1, 5], {\"a\": \"a-val\"}]").read(
                "$..['a', 'c']");
    ...
    public void scan_for_single_property() {
        ...
        Assertions.assertThat(JsonPath.parse(json).read("$..['a']", List.class)).containsExactly("aa", a, "aa");
    }
    ...
    public void scan_for_property_path() {
        ...
        Assertions.assertThat(JsonPath.parse(json).read("$..['a'].x", List.class)).containsExactly("xx", "xx");
    }
    ...
    public void scan_for_property_path_missing_required_property() {
        ...
        Assertions.assertThat(JsonPath.using(JSON_SMART_CONFIGURATION.addOptions(Option.REQUIRE_PROPERTIES)).parse(json).read("$..['a'].x", List.class)).containsExactly("xx", "xx");
    }
    ...
    public void scans_can_be_filtered() {
        ...
        Assertions.assertThat(JsonPath.using(JSON_SMART_CONFIGURATION.addOptions(Option.REQUIRE_PROPERTIES)).parse(animals).read("$..[?(@.mammal == true)].color", List.class)).containsExactly(brown, white);
    }
