...
import com.gg.jsonpath.spi.mapper.MappingException;
...
public class GsonJsonProviderTest extends BaseTest {
    ...
    public void json_can_be_parsed() {
        JsonObject node =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$");
        assertThat(node.get("string-property").getAsString()).isEqualTo("string-value");
    }

    @Test
    public void strings_are_unwrapped() {
        JsonElement node =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$.string-property");
        String unwrapped =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$.string-property", String.class);
    ...
    public void ints_are_unwrapped() {
        JsonElement node =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$.int-max-property");
        int unwrapped =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$.int-max-property", int.class);
    ...
    public void longs_are_unwrapped() {
        JsonElement node =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$.long-max-property");
        long val =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$.long-max-property", Long.class);
    ...
    public void doubles_are_unwrapped() {
        ...
        JsonElement node =  JsonPath.using(GSON_CONFIGURATION).parse(json).read("$.double-property");
        Double val =  JsonPath.using(GSON_CONFIGURATION).parse(json).read("$.double-property", Double.class);
    ...
    public void bigdecimals_are_unwrapped() {
        ...
        JsonElement node =  JsonPath.using(GSON_CONFIGURATION).parse(json).read("$.bd-property");
        BigDecimal val =  JsonPath.using(GSON_CONFIGURATION).parse(json).read("$.bd-property", BigDecimal.class);
    ...
    public void bigintegers_are_unwrapped() {
        ...
        JsonElement node =  JsonPath.using(GSON_CONFIGURATION).parse(json).read("$.bi-property");
        BigInteger val =  JsonPath.using(GSON_CONFIGURATION).parse(json).read("$.bi-property", BigInteger.class);
    ...
    public void int_to_long_mapping() {
        Assertions.assertThat(JsonPath.using(GSON_CONFIGURATION).parse("{\"val\": 1}").read("val", Long.class)).isEqualTo(1L);
    }

    @Test
    public void an_Integer_can_be_converted_to_a_Double() {
        Assertions.assertThat(JsonPath.using(GSON_CONFIGURATION).parse("{\"val\": 1}").read("val", Double.class)).isEqualTo(1D);
    }

    @Test
    public void list_of_numbers() {
        JsonArray objs =  JsonPath.using(GSON_CONFIGURATION).parse(JSON_DOCUMENT).read("$.store.book[*].display-price");
